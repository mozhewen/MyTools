...
Addtional definitions for PD[II[__], _] should be designated in specific problems. ";
PD[expr_Plus, x_] := PD[#, x]&/@expr
PD[c_ a_, x_] := PD[c, x] a + c PD[a, x]
PD[expr_?(FreeQ[#, _II]&), x_^i_.] := Cancel[1/(i x^(i-1)) \!\(
\*SubscriptBox[\(\[PartialD]\), \(x\)]\((expr)\)\)]
PD[ii_II, x_^i_] := Collect[1/(i x^(i-1)) PD[ii, x], _II]
...
RunFIRE::usage = 
"RunFIRE[pn, problemName, intName, iiList] uses the files generated by \
GenFIREFiles[pn, problemName, basis] to run the C++ version of FIRE to perform IBP reduction for iiList. ";
Options[RunFIRE] = {
	"Threads" -> 8,
	"Preferred" -> {},
	"OutDir" :> NotebookDirectory[]
...
ClearAll[subker]
RunFIRE[pn_Integer, problemName_String, intName_String, iiList_List, OptionsPattern[]] := 
	Module[{
		...
		Export[FileNameJoin[{OutDir, StringTemplate["``.wl"][intName]}], {pn, List@@#}&/@iiList];
...
|>],
		...
		(* 3. Get results *)
		ParallelEvaluate[
			Global`Tables2Rules[FileNameJoin[{OutDir, StringTemplate["``.tables"][intName]}], Factor]/.{Global`G[_, {as__}] :> II[as]},
			subker,
...
RunKira::usage = 
"RunKira[topoName, iiList] uses the files generated by GenKiraFiles[topoName, basis] to run Kira to perform \
the IBP reduction for iiList. Due to the feature of Kira, it's better to check that no scaleless \
...
RunKira[topoName_String, iiList_List, OptionsPattern[]] := 
	Module[{
			Threads = OptionValue["Threads"],
			rs = OptionValue["rs"],
			Preferred = OptionValue["Preferred"],
